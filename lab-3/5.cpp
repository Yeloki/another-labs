...
double f(double x) { return x / (pow(x, 4) + 81); }
...
int main() {
  ...
  // Вычисление интегралов по методам левых прямоугольников, трапеций и Симпсона
  // при шаге h1
  ...
  // Вывод результатов
  cout << "Интеграл методом левых прямоугольников: " << fixed << setprecision(6)
       << I_left_h2 << endl;
  cout << "Погрешность по методу Рунге-Ромберга: " << fixed << setprecision(6)
       << R_left << endl;

  cout << "Интеграл методом трапеций: " << fixed << setprecision(6) << I_trap_h2
       << endl;
  cout << "Погрешность по методу Рунге-Ромберга: " << fixed << setprecision(6)
       << R_trap << endl;

  cout << "Интеграл методом Симпсона: " << fixed << setprecision(6)
       << I_simpson_h2 << endl;
  cout << "Погрешность по методу Рунге-Ромберга: " << fixed << setprecision(6)
       << R_simpson << endl;
