...
vector<vector<double>> cubicSpline(const vector<double> &x,
...
double evaluateSpline(const vector<vector<double>> &coefficients,
                      const vector<double> &x, double x_star) {
  int n = x.size() - 1;
  int interval = 0;
  ...
  return coefficients[interval][0] + coefficients[interval][1] * dx +
...
int main() {
  ...
  cout << "Интерполяция кубическим сплайном: " << fixed << setprecision(6)
       << spline_result << endl;
