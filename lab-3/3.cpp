...
// Функция для решения нормальной системы уравнений для многочлена первой
...
// Функция для решения нормальной системы уравнений для многочлена второй
// степени (квадратичная регрессия)
vector<double> fitQuadratic(const vector<double> &x, const vector<double> &y) {
  ...
  vector<vector<double>> A = {{static_cast<double>(n), sumX, sumX2},
                              {sumX, sumX2, sumX3},
                              {sumX2, sumX3, sumX4}};
...
double calculateSSE(const vector<double> &x, const vector<double> &y,
                    const vector<double> &coeffs, int degree) {
  double sse = 0.0;
...
int main() {
  ...
  cout << "a0 = " << quadCoeff[0] << ", a1 = " << quadCoeff[1]
       << ", a2 = " << quadCoeff[2] << endl;
  ...
  cout << "Сумма квадратов ошибок для многочлена 1-ой степени: " << linearSSE
       << endl;
  cout << "Сумма квадратов ошибок для многочлена 2-ой степени: " << quadSSE
       << endl;
