...
double g(double x) { return sqrt(sin(x) + 1); }

// Метод простой итерации
double simpleIterationMethod(double (*g)(double), double initial_guess,
...
// Метод Ньютона
double newtonMethod(double (*f)(double), double (*df)(double),
                    double initial_guess, double tol, int &iterations) {
  double x_prev = initial_guess;
...
int main() {
  ...
  double root_simple_iteration =
      simpleIterationMethod(g, initial_guess, tol, iterations);
