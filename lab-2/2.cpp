...
void simpleIterationMethod(double x1_initial, double x2_initial, double tol, int& iterations, double& x1, double& x2) {
    x1 = x1_initial;
    x2 = x2_initial;
    ...
    while (true) {
        ...
        if (fabs(x1_new - x1) < tol && fabs(x2_new - x2) < tol) break;
...
// Метод Ньютона
void newtonMethod(double x1_initial, double x2_initial, double tol, int& iterations, double& x1, double& x2) {
    x1 = x1_initial;
    x2 = x2_initial;
    ...
    while (true) {
        ...
        if (fabs(dx1) < tol && fabs(dx2) < tol) break;
    }
}
