...
// Точное решение
double exactSolution(double x) { return 1.0 + x + log(fabs(x)); }

// Функция для метода стрельбы
void rungeKutta(double x0, double y0, double yp0, double h, int n,
                vector<double> &x, vector<double> &y, vector<double> &yp) {
  x[0] = x0;
  y[0] = y0;
  yp[0] = yp0;
  for (int i = 1; i < n; ++i) {
    double k1y = yp[i - 1];
    double k1yp = (x[i - 1] * yp[i - 1] - y[i - 1]) /
                  (x[i - 1] * x[i - 1] * log(fabs(x[i - 1])));

    double k2y = yp[i - 1] + 0.5 * h * k1yp;
    double k2yp = ((x[i - 1] + 0.5 * h) * (yp[i - 1] + 0.5 * h * k1yp) -
                   (y[i - 1] + 0.5 * h * k1y)) /
                  ((x[i - 1] + 0.5 * h) * (x[i - 1] + 0.5 * h) *
                   log(fabs(x[i - 1] + 0.5 * h)));

    double k3y = yp[i - 1] + 0.5 * h * k2yp;
    double k3yp = ((x[i - 1] + 0.5 * h) * (yp[i - 1] + 0.5 * h * k2yp) -
                   (y[i - 1] + 0.5 * h * k2y)) /
                  ((x[i - 1] + 0.5 * h) * (x[i - 1] + 0.5 * h) *
                   log(fabs(x[i - 1] + 0.5 * h)));
    ...
    double k4yp =
        ((x[i - 1] + h) * (yp[i - 1] + h * k3yp) - (y[i - 1] + h * k3y)) /
...
// Метод стрельбы
void shootingMethod(double x0, double xn, double y0, double ypn, double h,
...
void finiteDifferenceMethod(double x0, double xn, int n, vector<double> &x,
                            vector<double> &y) {
  double h = (xn - x0) / (n - 1);
  ...
  y[n - 1] = (D[n - 1] - A[n - 1] * beta[n - 2]) /
             (A[n - 1] * alpha[n - 2] + B[n - 1]);
  for (int i = n - 2; i >= 0; --i) {
...
int main() {
  ...
  // Вывод результатов
  cout
      << "x       Shooting Method    Finite Difference Method    Exact Solution"
      << endl;
  ...
  cout << "Погрешность конечно-разностного метода (h1 = 0.1): " << R_fdm
       << endl;
