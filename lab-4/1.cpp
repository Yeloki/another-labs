...
double exactSolutionDerivative(double x) {
    return 1.0 / x;
}

// Метод Эйлера
void eulerMethod(vector<double>& x, vector<double>& y, vector<double>& yp, double h) {
    int n = x.size();
    for (int i = 1; i < n; ++i) {
        double x_prev = x[i - 1];
        double y_prev = y[i - 1];
...
void rungeKuttaMethod(vector<double>& x, vector<double>& y, vector<double>& yp, double h) {
    int n = x.size();
    for (int i = 1; i < n; ++i) {
        double xi = x[i - 1];
        double yi = y[i - 1];
...
void adamsMethod(vector<double>& x, vector<double>& y, vector<double>& yp, double h) {
    int n = x.size();
    ...
    for (int i = 4; i < n; ++i) {
        y[i] = y[i - 1] + h * (55 * yp[i - 1] - 59 * yp[i - 2] + 37 * yp[i - 3] - 9 * yp[i - 4]) / 24.0;
        yp[i] = yp[i - 1] + h * (55 * ypp[i - 1] - 59 * ypp[i - 2] + 37 * ypp[i - 3] - 9 * ypp[i - 4]) / 24.0;
        ypp[i] = -(1.0 / x[i]) * yp[i];
    }
}
...
int main() {
    ...
    // Вывод результатов
    cout << "x       Euler Method    Runge-Kutta Method    Adams Method    Exact Solution" << endl;
    cout << fixed << setprecision(6);
    for (int i = 0; i < n1; ++i) {
        double exact = exactSolution(x1[i]);
        cout << x1[i] << "    " << y1[i] << "         " << y2[i] << "            " << y3[i] << "         " << exact << endl;
    }
