...
void luSolve(const vector<vector<double>>& LU, const vector<int>& P, const vector<double>& b, vector<double>& x, int n) {
    vector<double> y(n);
...
double luDeterminant(const vector<vector<double>>& LU, const vector<int>& P, int n) {
    double det = 1.0;
    for (int i = 0; i < n; ++i) {
        det *= LU[i][i];
        if (P[i] != i) {
...
vector<vector<double>> luInverse(const vector<vector<double>>& LU, const vector<int>& P, int n) {
