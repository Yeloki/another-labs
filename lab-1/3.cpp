...
void simpleIterationMethod(const vector<vector<double>> &A,
                           const vector<double> &b, double tol,
                           vector<double> &x, int &iterations) {
  int n = A.size();
...
void gaussSeidelMethod(const vector<vector<double>> &A, const vector<double> &b,
                       double tol, vector<double> &x, int &iterations) {
  int n = A.size();
  vector<double> x_old(n, 0.0);
  iterations = 0;
...
int main() {
  vector<vector<double>> A = {
      {-14, 6, 1, -5}, {-6, 27, 7, -6}, {7, -5, -23, -8}, {3, -8, -7, 26}};
